...
class mixed_emotion(Dataset):
    def __init__(self, root_dir, stage, configs, transform=None):
        ...
        if len(self._images) == 0 and self._is_cached:
            for image_path in self._image_paths:
                ...
                self._images.append(cv2.imread(os.path.join(self._image_dir, image_path)))

            np.save(os.path.join(self._root_dir, '{}.npy'.format(self._stage)), self._images)
    ...
    def _read_info(self):
        with open(os.path.join(self._root_dir, '{}.json'.format(self._stage))) as info_file:
            return json.load(info_file)

    def _is_npy(self):
        return (self._is_cached and os.path.exists(os.path.join(self._root_dir, '{}.npy'.format(self._stage))))
...
class mixed_infer(Dataset):
    ...
    def _read_info(self):
        return sorted(glob.glob(os.path.join(self._image_dir, '**/*.png'), recursive=True))
